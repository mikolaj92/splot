...
def _mojo_env() -> dict[str, str]:
    ...
    for root in candidates:
        ...
        if mojo_bin.is_file() and import_path.is_dir():
            ...
            # Force the detected pixi SDK home. A stale MODULAR_HOME=/opt/modular
            # in the parent env makes `mojo build` try to create /opt/modular.
            env["CONDA_PREFIX"] = str(root)
            env["MODULAR_HOME"] = str(root / "share" / "max")
            return env
    try:
